...
def main(point, folder, name, y, pass_index):
  ## Which bands to select
  bands = ['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B10', 'B11']
  ## build string for date filter of imagecollection on line 37
  base = datetime.date(y, 1, 1)
  inc = datetime.timedelta(16)
  start = str(base + pass_index * inc)
  end = str(base + (pass_index +1) * inc)
  print(start, end)
  ...
  task = geetools.batch.Export.image.toDrive(
    ...
    maxPixels=maxPixels)
  task.start()
  if pass_index==10:#  Just need one per year so choosing smack in the middle of growth just to hedge
...
for year in YEARS:
    ...
    for coord,name in zip(coords, loc_names):
        ...
        for pass_index in range(23):
            print(name, year, pass_index)
            main(point, 'earth_engine_high_temporal', name, year, pass_index)
            sys.stdout.flush()
